...
export class AliasCommands {
    ...
    public async alias(@Context() [interaction]: SlashCommandContext, @Options() { activity, alias }: AliasDto) {
        const updatedAlias = await this.aliasService.upsertAlias(interaction.guildId, activity, alias);
    ...
    public async unalias(@Context() [interaction]: SlashCommandContext, @Options() { activity }: UnaliasDto) {
        const deletedAlias = await this.aliasService.deleteAlias(interaction.guildId, activity);
        return interaction.reply({
    ...
    public async aliases(@Context() [interaction]: SlashCommandContext) {
        ...
        const aliasList = aliases.map(({ activity, alias }) => `\`${activity}\` -> \`${alias}\``).join("\n");
        return interaction.reply({
            content: `Aliases:\n${aliasList}`,
        })
    }
